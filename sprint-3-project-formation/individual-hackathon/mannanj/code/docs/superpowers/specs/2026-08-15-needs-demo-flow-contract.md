...
| Optional contact sharing | Review screen | Yes; name, email, and address are independently opt-in | Reducer and unit flow tests |
...
| Verify need | Review screen | Yes | Unit flow test |
| Review and mock-send | Verified need card | Yes | Unit flow test |
...
| Verified items | Verified needs | 3 | Verified or sent card | Opens send review | Stored locally |
| Inferred items | Unverified needs | 4 | Compact outlined card | Opens review | Mock discovery |
...
| Contact details | Include with this need | 4 | Three unchecked choices | Independently opt in | Optional; only selected fields are shared |
| Reject inference | Discard | Final left action | Red × plus link | Immediate local transition | Undo available at home |
| Accept inference | Verify | Final right action | Green check plus label | Immediate local transition | Disabled if a required value is blank |
...
| Home/discovered | Actions, updates, empty Verified needs, unverified card | Review, profile/reset | Reducer plus local storage | Review, welcome |
| Review | Back, title, editable values, optional contact choices, notes, Discard, Verify | Edit, select contact fields, discard, verify, back | Reducer draft persisted locally | Home |
...
| Review → Review contact selected | Toggle a contact checkbox | Checkbox state updates | Selected field becomes eligible for the share document | Checkbox | Toggle again to remove |
| Review → Home verified | Verify | Need moves to Verified needs | Status becomes confirmed | Home content | Reset |
...
- Accessible names and state announcements: icon-only × is named “Dismiss this need”; contact checkboxes have explicit labels; profile exposes expanded state; notices use status semantics.
